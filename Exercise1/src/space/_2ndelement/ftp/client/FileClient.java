...
public class FileClient {
    ...
    public void run() {
        try {
            ...
            // 接受第一条确认连接信息
            String confirmMsg = readMessage();
            if (!Constants.CONNECT_SUC_MARK.equals(confirmMsg)) {
                stderr.println("连接异常,未收到服务端回应");
            } else {
                sendMessage(String.valueOf(datagramSocket.getLocalPort()));
                String command;
     ...
     * {@link ServiceHandler#sendFile}
     */
    public void receiveFile() throws IOException {
        // 获取文件名
        String[] filenameAndLength = readMessage().split(Constants.DELIM);
        ...
        stdout.println();
        stdout.printf("接收完毕, 共 \u001b[35m%s\u001b[0m\n", Utils.formatVolume(totalSize));
        fos.close();
    }
