    ...
    public static void main(String[] args) throws Exception {
        try {
            ...
            if (request.startsWith("GET")) {
                 ...
                 * And ask the user to input the putted filename.
                 */
                screen.println("Enter the name of the file to upload:");
                String filename = keyboard.readLine();
                File uploadFile = new File(filename);
                if (!uploadFile.exists()) {
                    screen.println("File not exists");
                    myClient.close();
                    return;
                }
                myClient.processPutRequest(request, uploadFile);
            } else {
                /**
